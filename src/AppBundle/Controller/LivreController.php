...
class LivreController extends FOSRestController
{
    ...
    */
    public function ajouterlivre(Request $request)
    {
        ...
        $data->setDescriptif('ahmed ben salem');
        $data->setISBN('dev Crea');
    ...
    */
    public function modfierlivre(Request $request, $id)
    {
        ...
        $data->setTitre('modfier titre');
        $data->setDescriptif('modifier descriptif');
        $data->setISBN('modifier isbns');
        $data->setDateedition(new \DateTime());
        $em = $this->getDoctrine()->getManager();
        $em->persist($data);
        $em->flush();
    ...
    * @Rest\Get("/book/{titre}")
    */
    public function listeparametrer($titre)
    {
        $em = $this->getDoctrine()->getManager();
        $repo = $em->getRepository(Livre::class);
